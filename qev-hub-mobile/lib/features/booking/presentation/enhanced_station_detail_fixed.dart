...
class _EnhancedStationDetailFixedState extends ConsumerState<EnhancedStationDetailFixed> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      print('DEBUG: Fetching mock chargers...');
      final chargers = await _getChargersForStation(widget.stationId);
      print('DEBUG: Mock chargers loaded - ${chargers.length} chargers');
      ...
      // Add mock amenities if station doesn't have them
      final amenitiesList = station.amenities ?? _getMockAmenities();
      print('DEBUG: Amenities list: $amenitiesList');

      // Count available chargers from mock data
      ...
      final maxPower = chargers.map((c) => c.powerOutputKw ?? 0).reduce((a, b) => a > b ? a : b);
  ...
  Future<List<Charger>> _getChargersForStation(String stationId) async {
    // Simulate chargers data for this station
    // In production, this would query the chargers table
    return [
      Charger(
        id: '$stationId-1',
        stationId: stationId,
  ...
  void _onChargerSelected(Charger charger) {
  ...
  void _onBookCharger(Charger charger) {
  ...
  Widget _buildChargerDetailsModal(Charger charger) {
