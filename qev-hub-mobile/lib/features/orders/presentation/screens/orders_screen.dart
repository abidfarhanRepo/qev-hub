...
/// Orders screen with Active and Past tabs
...
class _ActiveOrdersTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    print('🔵 [_ActiveOrdersTab] Building - state: ${ordersAsync.value?.length ?? 0} orders');
    ...
    return RefreshIndicator(
      ...
      child: ordersAsync.when(
        data: (orders) {
          print('📊 [_ActiveOrdersTab] Displaying ${orders.length} active orders');
          if (orders.isEmpty) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              return _OrderCard(order: orders[index], isActive: true);
            },
          );
        },
        ...
        error: (error, _) => _buildErrorState(
          ...
          onRetry: () => ref.read(activeOrdersProvider.notifier).refresh(),
        ),
      ),
    );
  }
...
class _PastOrdersTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    print('🔵 [_PastOrdersTab] Building - state: ${ordersAsync.value?.length ?? 0} orders');

    return RefreshIndicator(
      onRefresh: () => ref.read(pastOrdersProvider.notifier).refresh(),
      color: AppColors.primary,
      backgroundColor: AppColors.surface,
      child: ordersAsync.when(
        data: (orders) {
          print('📊 [_PastOrdersTab] Displaying ${orders.length} past orders');
          if (orders.isEmpty) {
            return _buildEmptyState(
              icon: Icons.history,
          ...
          return ListView.builder(
            ...
            itemCount: orders.length,
            itemBuilder: (context, index) {
              return _OrderCard(order: orders[index], isActive: false);
            },
          );
        },
        loading: () => const _LoadingState(),
        error: (error, _) => _buildErrorState(
          message: 'Failed to load orders',
          onRetry: () => ref.read(pastOrdersProvider.notifier).refresh(),
        ),
      ),
    );
  }
                ...
                child: SizedBox(
                  ...
                  child: OutlinedButton(
                    onPressed: () {
                      ...
                      print('🔵 [_OrderCard] Navigate to order detail: ${order.id}');
